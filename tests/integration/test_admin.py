...
class TestUser:
    def test_create_delete(self, artifactory):
        ...
        test_user = User(artifactory=artifactory, name=user_name, email='test_user@example.com',
                         password='password')
...
class TestTargetPermission:
    ...
    def test_add_user_group(self, permission, user1, user2, group1):
        ...
        assert PermissionTarget.ADMIN not in permission.raw['principals']['users']['user1']

        permission.add_user(user2, PermissionTarget.ADMIN)
        permission.update()
        assert [PermissionTarget.ADMIN] == permission.raw['principals']['users']['user2']
